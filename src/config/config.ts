...
const zodSchema = z.object({
  ...
  ACCESS_TOKEN_SECRET: z.string().describe('access token secret key').min(16),
  ...
  EMAIL_VERIFICATION_FRONTEND_URL: z
    .string()
    .url()
    .describe('the url to the email verification page on the the front-end app'),
...
const config = {
  ...
  dbUrl: envVars.DATABASE_URL + (envVars.NODE_ENV === 'test' ? '-test' : ''),
  redisUrl: envVars.REDIS_URL,
  port: envVars.PORT,
  accessToken: {
    secret: envVars.ACCESS_TOKEN_SECRET,
    maxAge: envVars.ACCESS_TOKEN_MAX_AGE,
  },
  refreshToken: {
